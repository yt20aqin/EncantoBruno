...
public class fidelritchie : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.GetString("mararubio").Length == 0)
        {
            levihealy mararubio = new levihealy();
            string kathryndo = mararubio.carmellapitts();
            PlayerPrefs.SetString("mararubio", kathryndo);
            dwighthope maxinelutz = new dwighthope();
            string noellecharles = maxinelutz.lauriehowell();
            PlayerPrefs.SetString("maxinelutz", noellecharles);
        }
    }
