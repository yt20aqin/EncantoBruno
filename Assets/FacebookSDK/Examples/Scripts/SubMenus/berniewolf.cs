...
namespace Facebook.Unity.Example
{
    ...
    internal sealed class berniewolf : dionstevens
    {
        ...
        protected override void ivylittle()
        {
            ...
            {
                this.cassiewu(typeof(fabiangallo));
            }

            if (this.Button("App Requests"))
            {
                this.cassiewu(typeof(wilfredaviles));
            }

            if (this.Button("Graph Request"))
            {
                this.cassiewu(typeof(shermanrogers));
            }

            if (Constants.IsWeb && this.Button("Pay"))
            {
                this.cassiewu(typeof(loyddiehl));
            }

            if (this.Button("App Events"))
            {
                this.cassiewu(typeof(georgelott));
            }

            if (this.Button("App Links"))
            {
                this.cassiewu(typeof(noedaniel));
            }

            if (Constants.IsMobile && this.Button("Access Token"))
            {
                this.cassiewu(typeof(leondejesus));
            }
        ...
        private void CallFBLogin(LoginTracking mode, Scope scopemask)
        {
            ...
            {
                FB.Mobile.LoginWithTrackingPreference(LoginTracking.ENABLED, scopes, "classic_nonce123", this.lyndabray);    
            }
            else 
            {
                FB.Mobile.LoginWithTrackingPreference(LoginTracking.LIMITED, scopes, "limited_nonce123", this.yvettesantana);
            }
        ...
        private void CallFBLoginForPublish()
        {
            ...
            FB.LogInWithPublishPermissions(new List<string>() { "publish_actions" }, this.lyndabray);
        }
        ...
        private void OnInitComplete()
        {
            ...
            string logMessage = string.Format(
                ...
                FB.IsLoggedIn,
                FB.IsInitialized);
            emersonirvin.diannemadden(logMessage);
            if (AccessToken.CurrentAccessToken != null)
            {
                emersonirvin.diannemadden(AccessToken.CurrentAccessToken.ToString());
            }
        }
        ...
        private void OnHideUnity(bool isGameShown)
        {
            ...
            emersonirvin.diannemadden("Is game shown: " + isGameShown);
        }
    }
}
