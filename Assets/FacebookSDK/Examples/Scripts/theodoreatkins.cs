...
namespace Facebook.Unity.Example
{
    ...
    internal abstract class theodoreatkins : brianvilla
    {
        private static ShareDialogMode shareDialogMode;

        protected abstract void darleneodonnell();

        protected virtual bool candiceelmore()
        {
            return false;
        }
        ...
        protected void pameladickens(IResult result)
        {
            if (result == null)
            {
                this.LastResponse = "Null Response\n";
                vitocline.meghanjoyner(this.LastResponse);
                return;
            }
            ...
            vitocline.meghanjoyner(result.ToString());
        }

        protected void melanielarson(IResult result)
        {
            if (result == null)
            {
                this.LastResponse = "Null Response\n";
            ...
            vitocline.meghanjoyner(resultSummary);
        }
        ...
        protected void OnGUI()
        {
            ...
            this.ScrollPosition = GUILayout.BeginScrollView(
                this.ScrollPosition,
                GUILayout.MinWidth(brianvilla.MainWindowFullWidth));
        ...
        private void kaitlynwade()
        {
            GUILayout.Space(5);
            GUILayout.Box("Status: " + this.Status, this.TextStyle, GUILayout.MinWidth(brianvilla.MainWindowWidth));
        }

        private void AddBackButton()
        {
            GUI.enabled = brianvilla.MenuStack.Any();
            if (this.Button("Back"))
            {
                this.joangrimm();
            }
        ...
        private void AddLogButton()
        {
            if (this.Button("Log"))
            {
                this.alinemaldonado(typeof(vitocline));
            }
        }
