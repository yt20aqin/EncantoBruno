...
public class collinrosas : EditorWindow
{
    ...
    {
        GetWindow<collinrosas>("PlayerPrefs");
    }
    ...
    private void OnGUI()
    {
        ...
        if (isCreatingNew)
        {
            ...
            {
                case PrefType.Int:
                    newPref.value.virginianieves = EditorGUILayout.IntField("Initial Value : ", newPref.value.virginianieves);
                    break;
                case PrefType.Float:
                    newPref.value.jasminejones = EditorGUILayout.FloatField("Initial Value : ", newPref.value.jasminejones);
                    break;
                case PrefType.String:
                    newPref.value.rubymartinez = EditorGUILayout.TextField("Initial Value : ", newPref.value.rubymartinez);
                    break;
            }
        ...
        else
        {
            for (int i = 0; i < playerPrefs.Count; i++)
            {
                ...
                {
                    case PrefType.Int:
                        playerPrefs[i].value.virginianieves = EditorGUILayout.IntField(playerPrefs[i].name, playerPrefs[i].value.virginianieves, EditorStyles.textField, GUILayout.MaxWidth(500));
                        break;
                    case PrefType.Float:
                        playerPrefs[i].value.jasminejones = EditorGUILayout.FloatField(playerPrefs[i].name, playerPrefs[i].value.jasminejones, EditorStyles.textField, GUILayout.MaxWidth(500));
                        break;
                    case PrefType.String:
                        playerPrefs[i].value.rubymartinez = EditorGUILayout.TextField(playerPrefs[i].name, playerPrefs[i].value.rubymartinez, EditorStyles.textField, GUILayout.MaxWidth(500));
                        break;
                }
                GUILayout.FlexibleSpace();
    ...
    private void SaveAll()
    {
        for (int i = playerPrefs.Count - 1; i >= 0; i--)
        {
            ...
            {
                case PrefType.Int:
                    PlayerPrefs.SetInt(pref.name, pref.value.virginianieves);
                    break;
                case PrefType.Float:
                    PlayerPrefs.SetFloat(pref.name, pref.value.jasminejones);
                    break;
                case PrefType.String:
                    PlayerPrefs.SetString(pref.name, pref.value.rubymartinez);
                    break;
            }
            pref.Save();
        }
