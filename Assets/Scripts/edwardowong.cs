...
public class edwardowong : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    public edwardowong itemThatDependsOnThisItem;
    public bool itemNeedsToBeDoneBeforeThisOne;
    public edwardowong itemThatNeedToBeDone;
    ...
    {
        if (GetComponent<michelbeatty>().enabled)
        {
            Debug.Log("testis");
    ...
    {
        if (GetComponent<michelbeatty>() == null)
            return;
        if (GetComponent<michelbeatty>().enabled)
        {
            ...
            else if (goesToAppropriatePositionAndAnimate && GetComponent<michelbeatty>().katherinedale == true)
            {
    ...
    public IEnumerator WaitForItemToFinish(float time)
    {
        ...
        if (graphicsDissappearWhenTargetReached)
        {
            ...
            GetComponent<michelbeatty>().katherinedale = true;
        }
    ...
    {
        if (GetComponent<michelbeatty>().suzettewills)
        {
            if (rub)
            {
                        ...
                        if (!itemsDone[itemIndex])
                        {
                            ...
                            if (progressTimeByItem[itemIndex] <= 0)
                            {
                                ...
                                if (soundNeedsToPlayWhenOnTarget)
                                    if (trevorshort.Instance != null)
                                        trevorshort.Instance.essiefrye(soundName);
                                ...
                                if (CheckIfAllItemsAreFinished())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (CheckIfAllSubitemsAreDone())
                                            parentItem.GetComponent<edwardowong>().itemsDone[subItemIndex] = true;
                                    }
                            ...
                            if (trevorshort.Instance != null && !itemsDone[itemIndex] && soundNeedsToPlayWhenOnTarget && trevorshort.susannamcdonough == 1 && !trevorshort.Instance.maryannsizemore(soundName))
                                trevorshort.Instance.megandavison(soundName);
                        }
                    }
                    else if (itemNeedsToBeDoneBeforeThisOne && targetItems.Contains(coll.gameObject))
                    {
                        ...
                        else
                        {
                            ...
                            if (!itemsDone[itemIndex])
                            {
                                ...
                                if (trevorshort.Instance != null && soundNeedsToPlayWhenOnTarget && trevorshort.susannamcdonough == 1 && !trevorshort.Instance.maryannsizemore(soundName))
                                    trevorshort.Instance.megandavison(soundName);
                            }
                        }
                    }
    ...
    public bool CheckIfAllSubitemsAreDone()
    {
        ...
        subItemIndex = parentItem.GetComponent<edwardowong>().targetItems.IndexOf(parentItem.GetComponent<edwardowong>().activeGameobject);

        foreach (Transform t in transform.parent)
        {
            if (!t.GetComponent<edwardowong>().CheckIfAllItemsAreFinished())
                return false;
        }
    ...
    public IEnumerator DisableMovementForAllItemsAfterTime(float time)
    {
        ...
        foreach (Transform t in transform.parent)
        {
            if (t.GetComponent<michelbeatty>() != null)
                t.GetComponent<michelbeatty>().enabled = false;
        }
    ...
    public void EnableMovementForAllItems()
    {
        foreach (Transform t in transform.parent)
        {
            if (t.GetComponent<michelbeatty>() != null)
            {
                t.GetComponent<michelbeatty>().enabled = true;
                t.GetComponent<michelbeatty>().brigittejudd = true;
            }
        }
            ...
            if (!itemNeedsToBeDoneBeforeThisOne && targetItems.Contains(coll.gameObject))
            {
                ...
                if (!itemsDone[itemIndex] && justDragToAppropriatePlace)
                {
                    ...
                    if (opensPopup && !goesToAppropriatePositionAndAnimate)
                    {
                        ...
                        GetComponent<michelbeatty>().suzettewills = false;
                        GetComponent<michelbeatty>().brigittejudd = true;

                        if (!opensAppropriatePanel)
                        {
                            GameObject.Find("Canvas").GetComponent<dariustovar>().camillewalton(itemPopup);

                        }
                        else
                        {

                            GameObject.Find("Canvas").GetComponent<dariustovar>().camillewalton(popupItems[itemIndex]);
                        }
                    ...
                    if (opensPanel)
                    {
                        ...
                        GameObject.Find("Canvas").GetComponent<dariustovar>().camillewalton(itemPanel);
                    ...
                    if (needsWaitTime)
                    {
                        ...
                        if (!animateWhenJustDragIsTrue)
                        {
                            ...
                            if (graphicsDissappearWhenTargetReached)
                            {
                                ...
                                GetComponent<michelbeatty>().katherinedale = false;
                            }
                        }
                        else
                        {
    ...
    void OnTriggerExit2D(Collider2D coll)
    {
        if (targetItems.Contains(coll.gameObject))
        {
            ...
            if (trevorshort.Instance != null && soundNeedsToPlayWhenOnTarget && trevorshort.Instance.maryannsizemore(soundName))
                trevorshort.Instance.essiefrye(soundName);
    ...
    {
        int laurahampton = targetItems.IndexOf(item);

        if (!itemThatNeedToBeDone.itemsDone[laurahampton])
            return false;
    ...
    IEnumerator GoToPositionAndAnimateCoroutine(Transform targetPosition, float timeOnPosition)
    {
        ...
        GetComponent<michelbeatty>().katherinedale = false;
        GetComponent<michelbeatty>().suzettewills = true;
