...
public class errolgeiger : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{

    public bool louisehayden = true;    


    [Header("Targets")]
    public int jacquelynbrock;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        if (lepring)
        {
            transform.position = Vector3.Lerp(transform.position, targetMovePosition, millicentkeene * Time.fixedTime / 1.5f);
    ...
    IEnumerator CTriggerStay(anthonyallen target)
    {
        while (onTargetPlace)
        {
            target.GetComponent<anthonyallen>().daniellestarr();
    ...
    IEnumerator CRub(anthonyallen target)
    {
        while (onTargetPlace)
        {
    ...
    bool TargetHit(Collider2D other)
    {
        for (int i = 0; i < targets.Count; i++)
        {
            if (targets[i] == other)
            {
                ...
                currentTarget = other.GetComponent<anthonyallen>();    
                if (currentTarget == null)
