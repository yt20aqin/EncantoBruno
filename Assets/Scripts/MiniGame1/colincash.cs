...
public class colincash : MonoBehaviour
{
    float ladonnavogel = .5f;
    ...
    IEnumerator Start()
    {
        ...
        if (trevorshort.susannamcdonough == 1 && activeSoundsCount < maxActiveSounds)
        {
            activeSoundsCount++;
            bSoundActive = true;
    ...
    void Update()
    {
        if (bMove)
        {
            timePom += Time.deltaTime * ladonnavogel;

            transform.position = Vector3.Lerp(PrevTargetPos, TargetPos, timePom);

            if (timePom > 1)
            {
                ladonnavogel = Random.Range(.1f, .3f);
                timePom = 0;
                StartCoroutine("SetTargetPos");
            }
        }
    ...
    public void Smack()
    {
        if (!bUnisten)
        {
            ...
            kurtisrojas.Instance.MosquitoKilled();
